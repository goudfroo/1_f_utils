...
def onefcorr(fitsfile, fit_by_channel=True, backsub=False, boxsize=32, 
   ...
   Masks sources to fit 1/f pattern. Optionally subtracts 2-D background prior to 1/f pattern fit.
...
def onefcorr_wfss(fitsfile, fit_by_channel=False, backsub=True, boxsize=32, 
                  bkgmethod='median', makebkg=False, makemask=False, user_mask=None, 
                  sigma_bgmask=3.0, sigma_1fmask=2.0):
   """
   Perform 1/f correction for JWST NIRISS WFSS data.
   Input is a level 1 calibrated NIRISS image (i.e., a _rate.fits file).
   Flatfields the data, subtracts a background, and masks sources to fit 1/f pattern.
   After subtracting 1/f pattern, multiplies flatfield back in.
   Uses 'SLOWAXIS' header keyword to determine detector orientation.
   ...
   Parameters:
   fitsfile - FITS file with input level 1 calibrated image (_rate.fits file)
   fit_by_channel - fit each of the 4 amps separately when full-frame (default = False, applicable to most WFSS data)
   backsub - Optionally subtract a smooth 2-D background prior to fitting 1/f pattern (default = True)
   boxsize - Size of square box used during fit of 2-D background (default = 32)
   bkgmethod - Statistical method to use to determine background level ('median' [the default] or 'mode')
   makebkg - Optionally create FITS file containing the fitted 2-D background (default = False)
   makemask - Optionally create FITS file containing the mask to flag sources (default = False)
   ...
   P. Goudfrooij, STScI, Oct 2024
   Originally based on image1overf by Chris Willott at chriswillott/jwst
   """
   mandatory_env_vars = {"CRDS_PATH", "CRDS_SERVER_URL"}
   ...
   dq = hdulist[3].data
   slowaxis = abs(hdulist[0].header['SLOWAXIS'])
   # If necessary, retrieve WFSS flat file from CRDS and divide science extension by it
   pmap = hdulist[0].header['CRDS_CTX']
   flatfile = api.get_best_references(pmap, hdr)['flat']
   if not os.path.exists(os.environ['CRDS_PATH'] + '/references/jwst/niriss/' + flatfile):
      api.cache_best_references(pmap, hdr, reftypes=['flat'])
   theflatfile = os.environ['CRDS_PATH'] + '/references/jwst/niriss/' + flatfile
   flatimg = fits.getdata(theflatfile, ext=1)
   with warnings.catch_warnings():
   ...
   if ((len(image) == 2048) & (len(image[0]) == 2048) & fit_by_channel):
      ...
      if slowaxis == 1:
         ...
         for i in range(len(image)):
            image[i,:] = image[i,:] - medians[i] + themean
   # Multiply flat field back into the image
   newimage = image * flatimg
   #
   # Output copy of input HDUList with 1/f-corrected image in 1st ['SCI'] extension
   # 
   newhdul = fits.HDUList(hdulist)
   newhdul[1].data = newimage
   return newhdul
